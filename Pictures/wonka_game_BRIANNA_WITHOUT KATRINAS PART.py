...
class WonkaApp:
    def __init__(self, master):
        ...
        mainframe.pack()
        startLbl = Label(mainframe,bg="black",fg="white",text="Willy Wonka",font=("Courier",12))
        startLbl.pack()
        var = tk.IntVar()
    ...
    def initializeGameRoom(self,img,displaytext,root):
        ...
        screenroom_img = self.one = ImageTk.PhotoImage(resizedimg)
...
def displayRoomImage(img):
    ...
    imgpath = Image.open(os.path.dirname(os.path.abspath(__file__)) + '//' + img)
    resizedimg = imgpath.resize((500,500))
    screenroom_img = garbageone = ImageTk.PhotoImage(resizedimg)
